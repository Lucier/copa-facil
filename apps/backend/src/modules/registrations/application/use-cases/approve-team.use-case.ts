...
@Injectable()
export class ApproveTeamUseCase {
  constructor(
  ...
  async execute(id: string, dto: ReviewTeamDto, reviewedBy: string): Promise<RegistrationRequestEntity> {
    ...
    if (reg.status === RegistrationStatus.APROVADO) {
      throw new AppError('Registration is already approved', 'INVALID_STATE', 422)
    }
    return this.repo.updateStatus(id, RegistrationStatus.APROVADO, reviewedBy, dto.reviewNote)
  }
}
