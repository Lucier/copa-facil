# Runbook de Operações — Copa Fácil
...
```
equipe-gaucha.copa-facil.com.br → X-Tenant-ID: equipe-gaucha
```
...
curl http://meu-clube.copa-facil.com.br/api/v1/health
```
...
echo "127.0.0.1 meu-clube.copa-facil.local" | sudo tee -a /etc/hosts
curl http://meu-clube.copa-facil.local/api/v1/health
```
...
**Dashboard:** Copa Fácil — API Overview → painel "Latência p95"
...
**Dashboard:** Copa Fácil — PostgreSQL & Tenants → painel "Uso do Pool (%)"
...
**Dashboard:** Copa Fácil — Filas BullMQ → painel "Falhas (5 min)"
...
**Dashboard:** Copa Fácil — Filas BullMQ → painel "Memória Redis Utilizada"
